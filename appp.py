...
# ANALYSIS FUNCTION
# =====================================================
def analyze_target(image_file, threshold_val=80, min_area=10, max_area=1000, use_adaptive=True):
...
with st.sidebar:
    ...
    st.caption("Adjust if shots aren't detected properly")
    ...
    threshold_value = st.slider(
        ...
        max_value=150,
        value=80,
        help="Lower = detect lighter holes, Higher = detect darker holes"
    )
    ...
    min_area = st.slider(
        ...
        max_value=100,
        value=10,
        help="Smaller holes need lower values"
    )
    ...
    max_area = st.slider(
        ...
        max_value=2000,
        value=1000,
        help="Larger holes need higher values"
    )
    
    use_adaptive = st.checkbox(
        "Use Adaptive Threshold",
        value=True,
        help="Better for images with uneven lighting"
    )
    
    st.divider()
    st.info("💡 **Tip:** If detection fails, try adjusting these values and re-analyzing")
...
# Footer
st.divider()
st.caption(" Shooting Analysis System")